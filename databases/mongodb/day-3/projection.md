...
- **Note:** You cannot include and exclude fields simultaneously in the same query projection. Exception: \_id:0 we can use
...
- **Behavior:** It matches documents if _each specified value_ exists **anywhere** in the array (but not necessarily in the same element).
- **Example Usage:**
  ```javascript
  db.comments.find({
